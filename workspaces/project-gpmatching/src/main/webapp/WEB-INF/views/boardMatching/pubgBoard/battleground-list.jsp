	...
	<div id="db-wrapper" class="toggled" class="toggled">
		...
		<div id="page-content">
			...
			<a href="/project-gpmatching/boardMatching/pubgBoard/battleground-write">
				<button type="button" class="btn btn-primary me-2">배틀그라운드 매칭 게시글 쓰기</button>
			</a>
					...
					<div class="card">
						...
						<table id="matching-list" class="table table-sm table-dark table-hover">
							<thead>
								...
								<tbody id="battleground-list">		
									<c:forEach var="matchingBoard" items="${ requestScope.matchingPubgList }">
...
<!-- 댓글 쓰기 모달 -->
	...
	$(function() {
		$('#battleground-list').on("click", '.btn-show-comment-modal', function(event) {
	...
	$(function(){
		...
		$('#search-btn').on("click", function(event){
			...
			self.location = "/project-gpmatching/boardMatching/pubgBoard/battleground-list?searchType=" + searchType + "&keyword=" + keyword;
	...
	$(function() {
		$('#dynamic-button-area').on("click", '.btn-matching-modal-close', function(event) {
			location.href = "battleground-list";
	...
	$(function() {
		$('#battleground-list').on("click", '.btn-show-commentList-modal', function(event) {
